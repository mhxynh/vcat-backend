...
class TestTestsMain(TestCase):
    ...
    def _build_event(
        self, method, path, body=None, path_params=None, query_params=None
    ):
        event = {
            "httpMethod": method,
            "path": path,
    ...
    def test_get_tests_by_id_returns_200(self, mock_repo):
        mock_repo.get_tests_by_id.return_value = {
            "test_id": "42",
    ...
    def test_get_tests_by_request_with_details_returns_200(self, mock_repo):
        mock_repo.get_tests_by_request_with_details.return_value = [
            {"test_id": "1", "tester_name": "Bob"}
        ]
        event = self._build_event(
            "GET", "/tests", query_params={"request_id": "100", "details": "true"}
        )
    ...
    def test_get_tests_by_request_id_only_returns_200(self, mock_repo):
        mock_repo.get_tests_by_request_id.return_value = [
            {"test_id": "1", "request_id": "100"}
        ]
    ...
    def test_get_tests_by_control_id_returns_200(self, mock_repo):
        mock_repo.get_tests_by_control_id.return_value = [
    ...
    def test_post_test_bad_vgcpid_returns_400(self, mock_repo):
        ...
        mock_repo.create.return_value = (
            None  # Simulating the subquery failing to find an ID
        )
        event = self._build_event("POST", "/tests", body=body)
        ...
        self.assertEqual(result["statusCode"], 400)
        self.assertIn(
            "Failed to create test. Verify that VGCPID exists.",
    ...
    def test_put_action_update_details_returns_200(self, mock_repo):
        ...
        mock_repo.update_details.return_value = {
            "test_id": "42",
    ...
    def test_put_action_assign_returns_200(self, mock_repo):
        mock_repo.update_assigned_tester.return_value = {
            "test_id": "42",
            "assigned_tester_id": "7",
        }
        event = self._build_event(
            "PUT",
            "/tests/42",
            body={"action": "assign", "assigned_tester_id": "7"},
            path_params={"test_id": "42"},
        )
    ...
    def test_put_action_assign_missing_tester_id_returns_400(self, mock_repo):
        event = self._build_event(
            "PUT", "/tests/42", body={"action": "assign"}, path_params={"test_id": "42"}
        )
        ...
        self.assertEqual(result["statusCode"], 400)
        self.assertIn(
            "assigned_tester_id is required", json.loads(result["body"])["error"]
        )

    @patch("functions.tests.main.TestRepository")
    def test_put_action_start_returns_200(self, mock_repo):
        mock_repo.start_test.return_value = {
            "test_id": "42",
            "status": "OET_IN_PROGRESS",
        }
        event = self._build_event(
            "PUT", "/tests/42", body={"action": "start"}, path_params={"test_id": "42"}
        )
    ...
    def test_put_action_update_dat_returns_200(self, mock_repo):
        mock_repo.update_dat_track.return_value = {
            "test_id": "42",
            "status": "DAT_IN_PROGRESS",
        }
        event = self._build_event(
    ...
    def test_put_action_update_oet_returns_200(self, mock_repo):
        mock_repo.update_oet_track.return_value = {
            "test_id": "42",
            "status": "OET_IN_PROGRESS",
        }
        event = self._build_event(
    ...
    @patch("functions.tests.main.TestRepository")
    def test_put_action_update_evidence_links_missing_payload_returns_400(
        self, mock_repo
    ):
        event = self._build_event(
            "PUT",
    ...
    def test_put_missing_test_id_returns_400(self):
        event = self._build_event(
            "PUT", "/tests", body={"action": "start"}, path_params={}
        )
        result = tests_main.lambda_handler(event, None)
        self.assertEqual(result["statusCode"], 400)
    ...
    def test_put_test_not_found_returns_404(self, mock_repo):
        ...
        event = self._build_event(
            "PUT", "/tests/99", body={"action": "start"}, path_params={"test_id": "99"}
        )
    ...
    def test_exception_not_null_but_not_control_id(self, mock_repo, mock_logger):
        ...
        mock_repo.create.side_effect = Exception(
            "violates not-null constraint on column some_other_column"
        )
        event = self._build_event(
            "POST",
            "/tests",
    ...
    def test_exception_duplicate_request_control_test_returns_422(
        ...
        duplicate_error = Exception(
            ...
            '"tests_unique_per_request_control_track"\n'
            "DETAIL: Key (request_id, control_id)=(100, 88389) already exists."
        )
        mock_repo.create.side_effect = duplicate_error
        event = self._build_event(
            "POST",
            "/tests",
            body={
    ...
    def test_exception_not_null_returns_400(self, mock_repo, mock_logger):
        mock_repo.create.side_effect = Exception(
            "violates not-null constraint on column control_id"
        )
        event = self._build_event(
            "POST",
            "/tests",
        ...
        self.assertEqual(result["statusCode"], 400)
        self.assertIn(
            "provided vgcpid does not exist", json.loads(result["body"])["error"]
        )
    ...
    def test_exception_foreign_key_returns_400(self, mock_repo, mock_logger):
        mock_repo.create.side_effect = Exception(
            "violates foreign key constraint tests_request_id_fkey"
        )
        event = self._build_event(
            "POST",
            "/tests",
    ...
    def test_put_action_update_evidence_links_not_list_returns_400(self, mock_repo):
        ...
        self.assertEqual(result["statusCode"], 400)
        self.assertIn(
            "evidence_links must be a list", json.loads(result["body"])["error"]
        )
    ...
    def test_post_test_non_manager_returns_403(self, mock_repo, mock_logger):
        ...
        mock_logger.log.assert_any_call(
            level="WARNING", message="Unauthorized test creation attempt"
        )
    ...
    def test_delete_test_non_manager_returns_403(self, mock_repo, mock_logger):
        ...
        mock_logger.log.assert_any_call(
            level="WARNING", message="Unauthorized test deletion attempt"
        )
        self.assertEqual(result["statusCode"], 403)
        self.assertIn("Forbidden", json.loads(result["body"])["error"])
    ...
    def test_hard_delete_completed_test_returns_409(self, mock_repo, mock_logger):
        ...
        self.assertEqual(result["statusCode"], 409)
        self.assertIn(
            "Cannot hard delete completed", json.loads(result["body"])["error"]
        )
    ...
    def test_put_action_update_status_returns_200(self, mock_repo):
        ...
        event = self._build_event(
            "PUT",
            "/tests/42",
            body={"action": "update_status", "status": "IN_REVIEW"},
            path_params={"test_id": "42"},
        )
