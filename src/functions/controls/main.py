...
def lambda_handler(event, context):
    ...
    if len(event) == 0:
        ...
        return http_response(StatusCodes.BAD_REQUEST, {"error": "No event data provided"})
    ...
    try:
        ...
        if method == Methods.GET and normalized_path == "/controls":
            ...
            return http_response(StatusCodes.OK, controls)
        ...
        if method == Methods.GET:
            ...
            if vgcpid is None:
                ...
                return http_response(StatusCodes.BAD_REQUEST, {"error": "VGCPID not provided"})

            control = CrudUtils.get_by_id("controls", "vgcpid", vgcpid)
            if not control:
                Logger.log(level=LogLevels.ERROR, message="Control not found", extra_fields={"vgcpid": vgcpid})
                return http_response(StatusCodes.BAD_REQUEST, {"error": "Control not found", "vgcpid": vgcpid})

            Logger.log(level=LogLevels.INFO, message="Returning control", extra_fields={"vgcpid": vgcpid})
            return http_response(StatusCodes.OK, control)
        ...
        if method == Methods.POST and normalized_path == "/controls":
            ...
            if missing:
                ...
                return http_response(StatusCodes.BAD_REQUEST, {"error": "Missing required fields", "missing": missing})
            ...
            return http_response(StatusCodes.OK, created)
        ...
        if method == Methods.PUT:
            ...
            if not body:
                ...
                return http_response(StatusCodes.BAD_REQUEST, {"error": "No update data provided"})
            ...
            if not updates:
                ...
                return http_response(StatusCodes.BAD_REQUEST, {"error": "No valid fields", "allowed": allowed_fields})
            ...
            return http_response(StatusCodes.OK, updated)

        Logger.log(level=LogLevels.WARNING, message="Method not allowed", extra_fields={"method": method, "path": normalized_path})
        return http_response(StatusCodes.BAD_REQUEST, {"error": f"Method {method} not allowed on path {normalized_path}"})
    except Exception as e:
        Logger.log(level=LogLevels.ERROR, message="Error in controls handler", extra_fields={"exception": str(e)})
        return http_response(StatusCodes.INTERNAL_SERVER_ERROR, {"error": str(e)})
